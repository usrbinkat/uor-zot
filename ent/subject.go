...
type Subject struct {
	...
	// SubjectType holds the value of the "subjectType" field.
	...
	for i := range columns {
		...
		case subject.FieldSubjectType:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field subjectType", values[i])
			} else if value.Valid {
				s.SubjectType = value.String
			}
		case subject.FieldSubject:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field subject", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &s.Subject); err != nil {
...
func (s *Subject) String() string {
	...
	builder.WriteString(s.SubjectType)
	builder.WriteString(", ")
	builder.WriteString("subject=")
	builder.WriteString(fmt.Sprintf("%v", s.Subject))
	builder.WriteByte(')')
...
func (s *Subject) NamedStatement(name string) ([]*Statement, error) {
...
func (s *Subject) appendNamedStatement(name string, edges ...*Statement) {
	if s.Edges.namedStatement == nil {
