...
//		GroupBy(subject.FieldSubjectType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Subject.Query().
//		Select(subject.FieldSubjectType).
//		Scan(ctx, &v)
...
func (sq *SubjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subject, error) {
	...
	if query := sq.withStatement; query != nil {
		if err := sq.loadStatement(ctx, query, nodes,
			func(n *Subject) { n.Edges.Statement = []*Statement{} },
			func(n *Subject, e *Statement) { n.Edges.Statement = append(n.Edges.Statement, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range sq.withNamedStatement {
		if err := sq.loadStatement(ctx, query, nodes,
			func(n *Subject) { n.appendNamedStatement(name) },
			func(n *Subject, e *Statement) { n.appendNamedStatement(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *SubjectQuery) loadStatement(ctx context.Context, query *StatementQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *Statement)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(subject.StatementTable)
		s.Join(joinT).On(s.C(statement.FieldID), joinT.C(subject.StatementPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(subject.StatementPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(subject.StatementPrimaryKey[1]))
...
func (sq *SubjectQuery) WithNamedStatement(name string, opts ...func(*StatementQuery)) *SubjectQuery {
	query := (&StatementClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
